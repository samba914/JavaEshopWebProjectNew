...
public class Controller extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter out = response.getWriter()) {
			...
			case "panier": {
				if (request.getParameter("action") != null) {
					((Map<Integer, Integer>) session.getAttribute("panier"))
							.remove(parseInt(request.getParameter("id")));
				}
				RequestDispatcher rd = request.getRequestDispatcher(page + ".jsp");
				rd.forward(request, response);
			}
			...
			case "Connexion_verif": {
				...
				if (user != null) {
					...
					sleep(700);
					// si l'utilisateur valider son panier il va etre rediger au panier sinon il
					// sera dans la page d accueil

					if ((Boolean) session.getAttribute("panierValide") == false
							|| (Boolean) session.getAttribute("panierValide") == null) {
			...
			case "updateQte": {
				...
				((Map<Integer, Integer>) session.getAttribute("panier")).put(parseInt(request.getParameter("id")),
						parseInt(request.getParameter("qte")));
				RequestDispatcher rd = request.getRequestDispatcher("panier" + ".jsp");
				rd.forward(request, response);
				break;
			}
			case "MesCommandes": {
				...
				if(request.getParameter("idCommande")!=null) {
					request.setAttribute("idCommande", Integer.parseInt(request.getParameter("idCommande")));
					...
					request.setAttribute("idCommande",liste.get(0).getNum());
					int idCommande =liste.get(0).getNum();
					Commande commandeTosee= service.getCommandeById(liste, idCommande);
					request.setAttribute("commande",commandeTosee );
				}else {
					request.setAttribute("idCommande",0);
				}
			...
			case "AjoutCommande": {
				...
				if ((User) session.getAttribute("UserConnecte") != null) {
					service.faireCommande(((Map<Integer, Integer>) session.getAttribute("panier")),
							((User) session.getAttribute("UserConnecte")).getId());
